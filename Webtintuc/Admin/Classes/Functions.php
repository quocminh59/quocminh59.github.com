...
// Phan trang
function SoTrang($id,$tenbang,$sl_mottrang){
	global $conn;
	$query = "select count(".$id.") as tong from ".$tenbang." ";
	$result = mysqli_query($conn,$query);
	$row = mysqli_fetch_assoc($result);
	$tong = $row['tong'];
	if($tong >= $sl_mottrang){
		$sotrang = ceil($tong/$sl_mottrang);
		return $sotrang;
	}
	return "";
}
function Phantrang($sl_mottrang,$id,$tenbang,$lt){
	global $conn;
	//Tong so ban ghi cua nhom tin
	$query = "select count(".$id.") as tong from ".$tenbang." ";
	$result = mysqli_query($conn,$query);
	$row = mysqli_fetch_assoc($result);
	$tong = $row['tong'];
	...
	if($tong > $sl_mottrang){
		...
		// Xac dinh ban ghi dau tien 
		$start = ($page-1)*$sl_mottrang;
		//Truy van va hien thi noi dung 
	}
	else{
		header('Location: index.php?lt='.$lt.'');
	}
}
// Quan ly the loai
